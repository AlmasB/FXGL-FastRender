 ...
 */
public class CanvasTestApp extends GameApplication {
     ...
     */
    private BlockingQueue<Canvas> fullBuffers = new ArrayBlockingQueue<>(BUFFER_SIZE);

    /**
     * Stores images that can be drawn into.
     */
    private BlockingQueue<Canvas> emptyBuffers = new ArrayBlockingQueue<>(BUFFER_SIZE);

    /**
     * Current active (visible) buffer.
     */
    private Canvas currentBuffer;
    ...
    protected void initGame() {
        ...
        for (int i = 0; i < BUFFER_SIZE; i++) {
            emptyBuffers.add(new Canvas(W, H));
        }
    ...
    private void updateAndDraw(Canvas buffer) {
        var g = buffer.getGraphicsContext2D();

        g.setFill(Color.BLACK);
        g.fillRect(0, 0, W, H);

        g.setFill(Particle.COLOR);
